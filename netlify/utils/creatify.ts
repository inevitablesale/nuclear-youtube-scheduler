...
export async function buildShortFromArticle(url: string, targetAudience: string, length: 15|30|60 = 15) {
  const scriptRes = await axios.post(`${BASE}/api/ai_scripts/`, {
    url, language: "en", video_length: length,
    script_styles: ["ThreeReasonsWriter"],
    target_audience: targetAudience
  }, { headers: H, timeout: 60000 });

  const raw = scriptRes.data?.generated_scripts?.[0]?.paragraphs
          || scriptRes.data?.generated_scripts?.[0]?.script
          || scriptRes.data?.generated_scripts?.[0] || "";
  if (!raw) throw new Error("Creatify: empty script");

  const override_script = fix(String(raw));
  const createRes = await axios.post(`${BASE}/api/link_to_videos/`, {
